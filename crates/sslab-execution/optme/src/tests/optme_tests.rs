...
fn optme_test(input_txs: Vec<SimulatedTransaction>, answer: Vec<Vec<u64>>, print_result: bool) {
...
async fn optimistic_optme_par_test(
    input_txs: Vec<SimulatedTransaction>,
    answer: Vec<Vec<u64>>,
    print_result: bool,
) {
...
async fn test_scenario_1() {
    ...
    optme_test(txs.clone(), answer.clone(), false);
    optme_par_test(txs.clone(), answer.clone(), false).await;
    optimistic_optme_test(txs.clone(), answer.clone(), false);
    optimistic_optme_par_test(txs, answer, false).await;
}
